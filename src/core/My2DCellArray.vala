...
public class My2DCellArray  : GLib.Object
{
    ...
    public int rows {get {return dimensions.width;}}
    private int cols {get {return dimensions.height;}}
    private CellState[,] data;

    public My2DCellArray (Dimensions dimensions, CellState init=CellState.UNKNOWN)
    {
    ...
    public void set_data_from_cell(Cell c) {data[c.row,c.col]=c.state;}

    public void set_data_from_rc(int r, int c, CellState s) {data[r,c]=s;}

    public CellState get_data_from_rc(int r, int c) {return data[r,c];}

    public Cell get_cell(int r, int c) {return {r,c,data[r,c]};}

    public void get_row(int row, ref CellState[] sa, int start=0)
    {
        for (int c=start;c<start+sa.length;c++) sa[c]=data[row,c];
    }

    public void set_row(int row, CellState[] sa, int start=0)
    {
        for (int c=0;c<sa.length;c++) data[row,c+start]=sa[c];
    }

    public void get_col(int col, ref CellState[] sa, int start=0)
    {
        for (int r=start;r<start+sa.length;r++) {sa[r]=data[r,col];}
    }

    public void set_col(int col, CellState[] sa, int start=0)
    {
        for (int r=0;r<sa.length;r++) {data[r+start,col]=sa[r];}
    }
